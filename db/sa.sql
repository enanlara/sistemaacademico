...
-- Wed 11 May 2016 09:12:29 AM BRT
...
CREATE TABLE IF NOT EXISTS `academico`.`matriz_curso` (
  ...
  CONSTRAINT `fk_cursos_has_disciplinas_disciplinas1`
    FOREIGN KEY (`macu_curs_id`)
    REFERENCES `academico`.`disciplina` (`disc_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `academico`.`matricula_por_curso` (
  ...
  INDEX `fk_mcur_matricula_idx` (`mcur_estu_matricula` ASC)  COMMENT '',
  CONSTRAINT `fk_mcur_curs_id`
    FOREIGN KEY (`mcur_curs_id`)
    REFERENCES `academico`.`curso` (`curs_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_mcur_matricula`
